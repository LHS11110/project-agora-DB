...
MSSQL_TABLE_CPP_SERVER="${MSSQL_TABLE_CPP_SERVER:-${MSSQL_TABLE_PYTHON_SERVER:-cpp_server}}"
MSSQL_TABLE_PYTHON_SERVER="$MSSQL_TABLE_CPP_SERVER"
...
if command -v sqlcmd &> /dev/null; then
  ...
  sqlcmd -S "$MSSQL_HOST,$MSSQL_PORT" -U sa -P "$MSSQL_SA_PASS" -C -I \
    -v DB_NAME="$MSSQL_DB" DB_USER="$MSSQL_USER" DB_PASSWORD="$MSSQL_PASS" \
       ...
       TABLE_CPP_SERVER="$MSSQL_TABLE_CPP_SERVER" TABLE_PYTHON_SERVER="$MSSQL_TABLE_CPP_SERVER" \
    -i "$SQL_FILE"
else
